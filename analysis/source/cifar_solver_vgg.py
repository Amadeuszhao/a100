...
class solver:
    ...
    def __init__(self, model, verbose, mini_batch, batch_size,data_path):
        self.model = model
    ...
    def solve_fp(self, gen):
        ...
        #ind = np.argsort(perm_predict_avg[:,1])[::-1]get_cmv
    ...
    def analyze_eachclass_expand(self, cur_class):
        ...
        self.verbose = False
        x_class, y_class = load_adversairal_samples(self.data_path)
        class_gen = build_data_loader(x_class, y_class)
        print('class loading successfully')
        print(x_class.shape)
    ...
    def hidden_permutation_test_all(self, gen, pre_class, prefix=''):
        # calculate the importance of each hidden neuron
        out = []
        print('hidden_permutation_test_all')
        for cur_layer in self.layer:
            model_copy = keras.models.clone_model(self.model)
            model_copy.set_weights(self.model.get_weights())
...
def load_adversairal_samples(data_path):
